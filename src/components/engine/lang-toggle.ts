 ...
 * `[data-i18n-en]` node, flips `data-lang` on the root + `<html lang>`, and
 * repoints `--ff-display`/`--ff-ui` at the theme's Tamil or English typeface.
 * It never touches layout or replays scroll reveals, so it's safe to fire
 * after the GSAP scene has already settled.
 */
