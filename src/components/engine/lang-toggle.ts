 ...
 * `[data-i18n-en]` node (and `href` on `[data-i18n-href-en]` links) and flips
 * `data-lang` on the root + `<html lang>`. It never touches layout or replays
 * scroll reveals, so it's safe to fire after the GSAP scene has already settled.
 */
export function setupLangToggle(root: HTMLElement, initial: Lang, onChange?: (lang: Lang) => void) {
